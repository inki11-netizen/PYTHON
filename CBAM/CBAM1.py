...
class Attention_module(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        b4 = self.layer5(x)  # f(x)
        m4 = self.max_pool(b4)
        a4 = self.avg_pool(b4)
        max4 = self.mlp2(m4)
        avg4 = self.mlp2(a4)
        sum4 = max4 + avg4
        ca4a = self.sigmoid(sum4)
        ca4b = ca4a * b4
        attention4_s = ca4b
        avg_out4 = torch.mean(attention4_s, dim=1, keepdim=True)
        max_out4 = torch.max(attention4_s, dim=1, keepdim=True)[0]
        ca4 = torch.cat([avg_out4, max_out4], dim=1)
        sa4a = self.spatial2(ca4)
        sa4b = self.sigmoid(sa4a)
        x = sa4b * attention4_s
        ...
        # attention3_c = F.relu(x+shortcut8)
        m3 = self.max_pool(b3)
        a3 = self.avg_pool(b3)
        max3 = self.mlp3(m3)
        avg3 = self.mlp3(a3)
        sum3 = max3 + avg3
        ca3a = self.sigmoid(sum3)
        ca3b = ca3a * b3
        attention3_s = ca3b
        avg_out3 = torch.mean(attention3_s, dim=1, keepdim=True)
        max_out3 = torch.max(attention3_s, dim=1, keepdim=True)[0]
        ca3 = torch.concat([avg_out3, max_out3], dim=1)
        sa3a = self.spatial3(ca3)
        sa3b = self.sigmoid(sa3a)
        x = sa3b * attention3_s
        ...
        b4 = self.layer15(x) #f(x)
        # attention4_c = F.relu(x+shortcut14)
        m4 = self.max_pool(b4)
        a4 = self.avg_pool(b4)
        max4 = self.mlp4(m4)
        avg4 = self.mlp4(a4)
        ...
        avg_out4 = torch.mean(attention4_s, dim=1, keepdim=True)
        max_out4 = torch.max(attention4_s, dim=1, keepdim=True)[0]
        ca4 = torch.concat([avg_out4, max_out4], dim=1)
        sa4a = self.spatial4(ca4)
        sa4b = self.sigmoid(sa4a)
        x = sa4b * attention4_s
